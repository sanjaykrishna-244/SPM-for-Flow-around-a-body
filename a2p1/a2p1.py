...
R2     = X**2 + Y**2
mask   = R2 > 0          # points outside the cylinder
...
pd.DataFrame(ug).to_csv(f'a2p1/r{n}/u_{n}.csv', index=False, header=False)
pd.DataFrame(vg).to_csv(f'a2p1/r{n}/v_{n}.csv', index=False, header=False)
...
with pd.ExcelWriter(f'a2p1/r{n}/panel_geometry_{n}.xlsx', mode='w', engine='openpyxl') as writer:
...
plt.grid(True)
plt.savefig(f"a2p1/r{n}/Vt_vs_theta_{n}.png", dpi=300, bbox_inches='tight')
plt.show()
...
plt.grid(True)
plt.savefig(f"a2p1/r{n}/Cp_vs_theta_{n}.png", dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(f"a2p1/r{n}/streamlines_{n}.png", dpi=300, bbox_inches='tight')
plt.show()
