...
import pandas as pd
# __ Input parameters _____________________________________________
U_inf = 1.5
alpha = 0.0
U      = U_inf * np.cos(alpha)
V      = U_inf * np.sin(alpha)
# __ Load geometry _________________________________________________
...
# ______ Build influence matrices _______________________
I = np.zeros((len(xc), len(xc)))
J = np.zeros((len(xc), len(xc)))
...
# __ Solve for source strengths ___________________________________
S = np.linalg.solve(I, -U_inf*np.cos(beta))

# __ Surface pressure coefficient _________________________________
Vt = U_inf*np.sin(beta) + np.dot(J, S)
Vt_up = Vt[yc >= 0]; Cp_up = 1 - (Vt_up / U_inf)**2
Vt_lo = Vt[yc < 0]; Cp_lo = 1 - (Vt_lo / U_inf)**2

# __ Velocity field (vectorised over panels) ______________________
Ng     = 400
...
pd.DataFrame(ug).to_csv(f'a2p2/SPM/u.csv', index=False, header=False)
pd.DataFrame(vg).to_csv(f'a2p2/SPM/v.csv', index=False, header=False)
...
with pd.ExcelWriter(f'a2p2/SPM/source_panel_method.xlsx', mode='w', engine='openpyxl') as writer:
...
"""
# ___ Cp and velocity field plots ___________________________________________
...
plt.figure()
plt.plot(x_upper[:-1], cp_upper[:-1], 'o-', label='Cp at control points (Upper Surface)')
plt.plot(x_lower[:-1], cp_lower[:-1], 's-', label='Cp at control points (Lower Surface)')
plt.gca().invert_yaxis()
plt.xlabel(f'x | N={len(xc)}');  plt.ylabel('Cp')
plt.title('Pressure Coefficient Distribution over NACA0012 — Source Panel Method')
plt.savefig(f'a2p2/SPM/Cp_distribution.png', dpi=300, bbox_inches='tight')
plt.show()
"""
plt.figure()
plt.plot(xc[1:-1], Vt[1:-1], 'o-', label='Tangential Velocity at control points')
plt.xlabel('x');  plt.ylabel('Tangential Velocity')
plt.title('Tangential Velocity Distribution over NACA0012 — Source Panel Method')
plt.savefig(f'a2p2/SPM/tangential_velocity_{len(xc)}.png', dpi=300, bbox_inches='tight')
plt.show()
"""
...
ax.set_title('Symmetric flow over NACA0012 hydrofoil — Source Panel Method', fontsize=13)
ax.set_xlabel('x');  ax.set_ylabel('y')
plt.tight_layout()
plt.savefig(f"a2p2/SPM/streamlines.png", dpi=300, bbox_inches='tight')
plt.show()